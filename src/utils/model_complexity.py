...
def hook_convNd(m, x, y):
    x = x[0]
    weight_mask = torch.where(m.weight == 0, torch.zeros_like(m.weight), torch.ones_like(m.weight))
    x_mask = torch.where(x == 0, torch.zeros_like(x), torch.ones_like(x))
    if isinstance(m, nn.Conv2d):
        r = torch.nn.functional.conv2d(x_mask, weight_mask, groups=m.groups, stride=m.stride, padding=m.padding)
    elif isinstance(m, nn.Conv1d):
        r = torch.nn.functional.conv1d(x_mask, weight_mask, groups=m.groups, stride=m.stride, padding=m.padding)
    elif isinstance(m, nn.Conv3d):
        r = torch.nn.functional.conv3d(x_mask, weight_mask, groups=m.groups, stride=m.stride, padding=m.padding)
    sparse_flops =  int(r.sum().item())
...
def hook_linear(m, x, y):
    ...
    weight_mask = torch.where(m.weight == 0, torch.zeros_like(m.weight), torch.ones_like(m.weight))
    x_mask = torch.where(x == 0, torch.zeros_like(x), torch.ones_like(x))
    r = torch.nn.functional.linear(x_mask, weight_mask)
    sparse_flops =  int(r.sum().item())
...
def compute_model_complexity(
    model, input, verbose=False, only_conv_linear=True
):
    ...
    registered_handles = []
    layer_list = []
    layer = namedtuple('layer', ['class_name', 'params', 'dense_flops', 'sparse_flops'])
    ...
    def _add_hooks(m):
        ...
        def _hook(m, x, y):
            ...
            if class_name in flops_counter:
                dense_flops, sparse_flops = flops_counter[class_name](m, x, y)
            else:
                dense_flops, sparse_flops = 0, 0
            layer_list.append(
                layer(class_name=class_name, params=params,
    ...
    if verbose:
        ...
        per_module_dense_flops = defaultdict(list)
        per_module_sparse_flops = defaultdict(list)

    total_params, total_dense_flops, total_sparse_flops = 0, 0, 0

    for layer in layer_list:
        total_params += layer.params
        total_dense_flops += layer.dense_flops
        total_sparse_flops += layer.sparse_flops
        if verbose:
            per_module_params[layer.class_name].append(layer.params)
            per_module_dense_flops[layer.class_name].append(layer.dense_flops)
            per_module_sparse_flops[layer.class_name].append(layer.sparse_flops)

    if verbose:
        num_udscore = 55
        print('  {}'.format('-' * num_udscore))
        print('  Model complexity with input size {}'.format(input.shape))
        print('  {}'.format('-' * num_udscore))
        for class_name in per_module_params:
            params = int(np.sum(per_module_params[class_name]))
            dense_flops = int(np.sum(per_module_dense_flops[class_name]))
            sparse_flops = int(np.sum(per_module_sparse_flops[class_name]))
            print(
                    '  {} (params={:,}, dense flops={:,}, sparse flops={:,})'.format(
                    class_name, params, dense_flops, sparse_flops
                )
            )
        print('  {}'.format('-' * num_udscore))
        print(
                '  Total (params={:,}, dense flops={:,}, sparse flops={:,})'.format(
                total_params, total_dense_flops, total_sparse_flops
            )
        )
        print('  {}'.format('-' * num_udscore))

    return total_params, total_dense_flops, total_sparse_flops
